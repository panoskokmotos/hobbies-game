...
import { Heart, X, ChevronRight, Download, Copy, Check, RefreshCw } from 'lucide-react'
...
const SEED_PROFILES = [
  ...
  },
  {
    ...
    interests: ['📈 Investing', '🎙️ Podcasting', '🌿 Foraging', '📸 Photography', '🎸 Guitar'],
...
function RadarChart({ scores, size = 280, animate = false }) {
  ...
  const dataPoints = CATEGORIES.map((cat, i) => {
    const val = Math.min(scores[cat] || 0, 6)
    const r = drawn ? (val / 6) * maxR : 0
    const p = toXY(i, r)
    return `${p.x},${p.y}`
  }).join(' ')
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      <polygon points={dataPoints} fill="rgba(251,191,36,0.18)" stroke="#fbbf24" strokeWidth="2"
...
function SwipeScreen({ onComplete }) {
  ...
  useEffect(() => {
    // Hint animation: slight tilt right, then back
    const t = setTimeout(() => {
      setHintPlayed(true)
      setOffset({ x: 18, y: 0 })
      setTimeout(() => setOffset({ x: 0, y: 0 }), 400)
    }, 900)
    return () => clearTimeout(t)
  }, [])
  ...
  if (exiting === 'right') {
    cardTransform = 'translateX(150vw) rotate(30deg)'
    cardTransition = 'transform 0.38s cubic-bezier(0.55,0,1,0.45)'
      ...
      <div className="w-full max-w-xs px-6 mb-6">
        ...
        <div className="w-full h-1 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.08)' }}>
          <div className="h-full rounded-full transition-all duration-300"
            style={{ width: `${progress * 100}%`, background: 'linear-gradient(90deg,#f59e0b,#fbbf24)' }} />
        </div>
        {liked.length > 0 && (
      ...
      <div className="relative" style={{ width: 320, height: 460 }}>
        {thirdCard && (
          <div className="absolute inset-0 rounded-3xl"
            style={{
              background: 'linear-gradient(145deg,#141428,#0e1020)',
        ...
        {nextCard && (
          <div className="absolute inset-0 rounded-3xl"
            style={{
              background: 'linear-gradient(145deg,#181830,#141428)',
        ...
        <div
          ...
          style={{
            background: 'linear-gradient(145deg,#1c1c34,#161628)',
        ...
        >
          ...
          <div className="absolute inset-0 rounded-3xl pointer-events-none overflow-hidden">
            <div style={{
              ...
              background: swipeDir === 'right'
                ? 'radial-gradient(circle at 30% 50%, rgba(251,191,36,0.12) 0%, transparent 60%)'
                : swipeDir === 'left'
                ? 'radial-gradient(circle at 70% 50%, rgba(59,130,246,0.12) 0%, transparent 60%)'
                : 'none',
          ...
          <div className="absolute top-8 left-7 pointer-events-none"
            style={{ opacity: rightOpacity, transform: 'rotate(-14deg)', transition: 'opacity 0.1s' }}>
            <div className="px-4 py-1.5 rounded-xl" style={{
              border: '2.5px solid #fbbf24', background: 'rgba(251,191,36,0.12)',
            }}>
          ...
          {/* NOT ME stamp */}
          <div className="absolute top-8 right-7 pointer-events-none"
            style={{ opacity: leftOpacity, transform: 'rotate(14deg)', transition: 'opacity 0.1s' }}>
            <div className="px-4 py-1.5 rounded-xl" style={{
              border: '2.5px solid #60a5fa', background: 'rgba(96,165,250,0.12)',
            }}>
          ...
          <div className="mb-4">
            <span className="px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-widest"
              style={{
                background: `${CATEGORY_COLORS[card.category]}18`,
                color: CATEGORY_COLORS[card.category],
                border: `1px solid ${CATEGORY_COLORS[card.category]}35`,
              }}>
              {CATEGORY_LABELS[card.category]}
            </span>
          </div>

          <div className="text-[90px] leading-none mb-5 pointer-events-none">{card.emoji}</div>
...
function ArchetypeScreen({ archetype, liked, onNext }) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 py-12 relative overflow-hidden"
      ...
      <div className="absolute inset-0 pointer-events-none animate-glow-pulse" style={{
        background: 'radial-gradient(ellipse 60% 40% at 50% 30%, rgba(251,191,36,0.14) 0%, transparent 70%)',
      }} />

      {/* Flash burst */}
      <div className="absolute inset-0 pointer-events-none transition-opacity duration-700"
        style={{
          background: 'radial-gradient(circle at 50% 40%, rgba(251,191,36,0.25) 0%, transparent 55%)',
          opacity: phase === 0 ? 1 : 0,
        }} />
      ...
      <div className="w-full max-w-sm relative z-10"
        ...
        <p className="text-center text-lg leading-relaxed mb-2" style={{ color: 'rgba(255,255,255,0.75)' }}>
          {archetype.description}
        </p>
        <p className="text-center text-base leading-relaxed mb-10" style={{ color: 'rgba(255,255,255,0.4)' }}>
          {archetype.description2}
        </p>
        ...
        <div style={{
          ...
          <div className="rounded-3xl p-4 mb-6" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div className="flex flex-wrap gap-2 justify-center mb-8">
              {topCats.map(cat => (
                <div key={cat} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium"
                  ...
                  <span style={{ opacity: 0.6 }}>×{scores[cat]}</span>
                </div>
              ))}
            </div>
          )}
...
function RecommendationsScreen({ liked, onNext }) {
  ...
  useEffect(() => {
    if (!hasFetched.current) fetchRecs(apiKey)
  }, [fetchRecs, apiKey])
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 gap-5">
            <div className="relative w-14 h-14">
              ...
              <div className="absolute inset-2 rounded-full border border-amber-400 border-b-transparent"
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div className="text-center">
                    <span className="font-bold text-lg"
                      style={{ fontFamily: 'Fraunces, serif', color: section.color, letterSpacing: '-0.02em' }}>
                      {section.title}
                    </span>
                ...
                <div className="space-y-2.5">
                  {data?.[section.key]?.map((item, i) => (
                    <div key={i} className="rounded-2xl p-4 flex items-start gap-3"
                      style={{
                        background: `${section.color}09`,
                        border: `1px solid ${section.color}20`,
                        opacity: phase ? 1 : 0,
                        transform: phase ? 'none' : 'translateY(8px)',
                        transition: `opacity 0.5s ${(si * 3 + i) * 60}ms, transform 0.5s ${(si * 3 + i) * 60}ms`,
                      }}>
    ...
    <div className="min-h-screen px-4 py-10" style={{ background: '#0a0a0f' }}>
      <div className="max-w-sm mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm" style={{ color: 'rgba(255,255,255,0.35)' }}>
            People whose curiosity interlocks with yours
          </p>
        </div>
          ...
          {SEED_PROFILES.map((profile, i) => (
            <div key={i} className="rounded-3xl p-5"
              style={{
                ...
                transition: `opacity 0.6s ${i * 120}ms, transform 0.6s cubic-bezier(0.16,1,0.3,1) ${i * 120}ms`,
              }}>
              ...
              <p className="text-xs italic mb-4 leading-relaxed" style={{ color: 'rgba(255,255,255,0.45)' }}>
                "{profile.why}"
              </p>
...
function WrappedCard({ archetype, liked, innerRef }) {
  ...
  return (
    <div ref={innerRef} style={{
      ...
      background: 'linear-gradient(145deg,#0a0a0f 0%,#1a1428 45%,#0a0a18 100%)',
      ...
      <div style={{ position:'relative', zIndex:1, textAlign:'center', width:'100%' }}>
        ...
        {top5.length > 0 && (
          <div style={{ display:'flex', justifyContent:'center', gap:10, marginBottom:28 }}>
            {top5.map((c, i) => (
              <span key={i} style={{ fontSize:30 }}>{c.emoji}</span>
            ))}
          </div>
        )}
        ...
        <div style={{
          ...
          color:'#fbbf24', fontSize:11, fontWeight:700, letterSpacing:1,
        }}>
          Top {archetype.rarity}% of curious minds
        </div>

        <div style={{ marginTop:24, fontSize:10, color:'rgba(255,255,255,0.2)', letterSpacing:2 }}>
          POLYMATH.APP
        </div>
      </div>
    </div>
...
function ProfileScreen({ archetype, liked, recommendations, onRestart }) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      alert('Right-click the card and select "Save Image As…"')
    }
    ...
    <div className="min-h-screen px-4 py-10" style={{ background: '#0a0a0f' }}>
      <div className="max-w-sm mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="mt-2 leading-relaxed text-sm" style={{ color: 'rgba(255,255,255,0.5)' }}>
            {archetype.description}
          </p>
        </div>
            ...
            <div className="flex flex-wrap gap-2">
              {liked.map(card => (
                <span key={card.id} className="px-3 py-1.5 rounded-full text-sm"
                  style={{
                    background: `${CATEGORY_COLORS[card.category]}12`,
                    color: CATEGORY_COLORS[card.category],
                    border: `1px solid ${CATEGORY_COLORS[card.category]}28`,
                  }}>
        ...
        {sections.map(section => section.items?.length > 0 && (
          <div key={section.key} className="mb-7">
            <h2 className="font-bold text-base mb-3"
              style={{ fontFamily: 'Fraunces, serif', color: section.color }}>
              {section.title}
            </h2>
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          <button onClick={handleCopy}
            className="py-3 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 transition-all duration-200 hover:scale-[1.02] active:scale-[0.98]"
            style={{
              background: copied ? 'rgba(16,185,129,0.12)' : 'rgba(255,255,255,0.05)',
              border: copied ? '1px solid rgba(16,185,129,0.35)' : '1px solid rgba(255,255,255,0.1)',
        ...
        <button onClick={onRestart}
          className="w-full py-3 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 mb-8 transition-all duration-200 hover:opacity-70"
    ...
    <div style={{ minHeight: '100vh', background: '#0a0a0f' }}>
      ...
      {screen === 'recommendations' && (
        <RecommendationsScreen
          liked={liked}
          onNext={data => { setRecommendations(data); setScreen('matches') }}
        />
      )}
      {screen === 'matches' && (
        <MatchesScreen
