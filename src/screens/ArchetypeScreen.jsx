    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-6 py-12 relative overflow-hidden"
      ...
      <div className="w-full max-w-sm relative z-10"
        ...
        <h1 className="text-center mb-4 leading-tight"
          style={{ fontFamily: 'Fraunces, serif', fontWeight: 800, fontSize: 36, letterSpacing: '-0.03em', color: TEXT }}>
          {archetype.name}
        </h1>
        <p className="text-center text-xs mb-4 font-medium" style={{ color: text(0.3) }}>
          {Math.round(21000 * archetype.rarity / 100)} Polymaths worldwide share this archetype
        </p>
        <p className="text-center text-lg leading-relaxed mb-2" style={{ color: text(0.75) }}>{archetype.description}</p>
        <p className="text-center text-base leading-relaxed mb-10" style={{ color: text(0.4) }}>{archetype.description2}</p>
        ...
        <div style={{ opacity: phase >= 2 ? 1 : 0, transform: phase >= 2 ? 'scale(1)' : 'scale(0.8)', transition: 'opacity 0.6s ease, transform 0.8s cubic-bezier(0.34,1.56,0.64,1)' }}>
          <div className="rounded-3xl p-4 mb-5" style={{ background: text(0.03), border: `1px solid ${text(0.06)}` }}>
        ...
        <button onClick={handleShare}
          ...
          style={{
            background: copied ? 'rgba(16,185,129,0.15)' : text(0.08),
            border: copied ? '1px solid rgba(16,185,129,0.4)' : `1px solid ${text(0.18)}`,
